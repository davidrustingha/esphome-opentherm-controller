    ...
    return switch.switch_schema().extend({
        ...
        cv.Optional(CONF_MODE, entity["default_mode"]): cv.enum({
            "restore_default_on": cg.RawExpression(
                "opentherm::OpenthermSwitchMode::OPENTHERM_SWITCH_RESTORE_DEFAULT_ON"
            ),
            "restore_default_off": cg.RawExpression(
                "opentherm::OpenthermSwitchMode::OPENTHERM_SWITCH_RESTORE_DEFAULT_OFF"
            ),
            "start_on": cg.RawExpression(
                "opentherm::OpenthermSwitchMode::OPENTHERM_SWITCH_START_ON"
            ),
            "start_off": cg.RawExpression(
                "opentherm::OpenthermSwitchMode::OPENTHERM_SWITCH_START_OFF"
            )
        })
